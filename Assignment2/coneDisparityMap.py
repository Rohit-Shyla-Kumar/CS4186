# -*- coding: utf-8 -*-
"""
Created on Sun Apr 14 23:56:02 2019
...
cones_l = cv2.imread("data/relative_height/1_a.jpg", cv2.IMREAD_GRAYSCALE)
cones_r = cv2.imread("data/relative_height/1_b.jpg", cv2.IMREAD_GRAYSCALE)
...
cv2.imwrite("cones_l_keypoints.jpg",img)
img =cv2.drawKeypoints(cones_r,kp2,outImage=np.array([]), color=(0, 0, 255))
plt.imshow(img),plt.colorbar(),plt.show()
cv2.imwrite("cones_r_keypoints.jpg",img)
...
img3 = cv2.drawMatchesKnn(cones_l,kp1,cones_r,kp2,good[:500],None,flags=2)
...
img5,img6 = drawlines(cones_l,cones_r,lines1,pts1,pts2)

# Find epilines corresponding to points in left image (first image) and
# drawing its lines on right image
lines2 = cv2.computeCorrespondEpilines(pts1.reshape(-1,1,2), 1,F)
lines2 = lines2.reshape(-1,3)
img3,img4 = drawlines(cones_r,cones_l,lines2,pts2,pts1)
...
cv2.imwrite('cones_l_epilines.jpg',img5)
cv2.imwrite('cones_r_epilines.jpg',img3)
...
retBool ,rectmat1, rectmat2 = cv2.stereoRectifyUncalibrated(p1fNew,p2fNew,F,cones_l.shape[:2])

dst_l = cv2.warpPerspective(cones_l,rectmat1,cones_l.shape[:2])
dst_r = cv2.warpPerspective(cones_r,rectmat2,cones_r.shape[:2])
